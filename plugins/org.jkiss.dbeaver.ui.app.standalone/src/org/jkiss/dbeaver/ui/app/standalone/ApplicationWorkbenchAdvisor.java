...
import org.jkiss.sqbase.core.CoreFeatures;
...
import org.jkiss.sqbase.model.app.DBPApplicationDesktop;
...
import org.jkiss.sqbase.model.task.DBTTaskManager;
...
import org.jkiss.sqbase.ui.app.standalone.internal.CoreApplicationMessages;
...
import org.jkiss.sqbase.ui.editors.content.ContentEditorInput;
import org.jkiss.sqbase.ui.perspective.SQBasePerspective;
import org.jkiss.sqbase.ui.preferences.PrefPageConnectionsGeneral;
import org.jkiss.sqbase.ui.preferences.PrefPageConstants;
import org.jkiss.sqbase.ui.preferences.PrefPageDatabaseEditors;
import org.jkiss.sqbase.ui.preferences.PrefPageDatabaseUserInterface;
...
public class ApplicationWorkbenchAdvisor extends IDEWorkbenchAdvisor {
    ...
    private static final String PERSPECTIVE_ID = SQBasePerspective.PERSPECTIVE_ID;
    public static final String DBEAVER_SCHEME_NAME = "org.jkiss.sqbase.defaultKeyScheme"; //$NON-NLS-1$

    @Deprecated(since = "25.1.5")
    private static final String PROP_PERSPECTIVE_VERSION = "sqbase.perspectiveVersion"; //$NON-NLS-1$
    @Deprecated(since = "25.1.5")
    private static final String PROP_WORKBENCH_VERSION = "sqbase.workbenchVersion"; //$NON-NLS-1$
    ...
    private static final Map<String, List<String>> fontOverrides = Map.of(
        UIFonts.SQBase.MONOSPACE_FONT, List.of(
            UIFonts.Eclipse.TEXT_EDITOR_BLOCK_SELECTION_FONT,
            UIFonts.Eclipse.TEXT_FONT,
            UIFonts.Eclipse.CONSOLE_FONT,
            UIFonts.Eclipse.DETAIL_PANE_TEXT_FONT,
        ...
        UIFonts.SQBase.MAIN_FONT, List.of(
            UIFonts.Eclipse.DIALOG_FONT,
            UIFonts.Eclipse.VARIABLE_TEXT_FONT,
            UIFonts.Eclipse.PART_TITLE_FONT,
            UIFonts.Eclipse.TREE_AND_TABLE_FONT_FOR_VIEWS
    ...
    public void initialize(IWorkbenchConfigurer configurer) {
        ...
        WorkbenchImages.getImageRegistry().put(IDEInternalWorkbenchImages.IMG_OBJS_ERROR_PATH, SQBaseIcons.getImageDescriptor(DBIcon.SMALL_ERROR));
        WorkbenchImages.getDescriptors().put(IDEInternalWorkbenchImages.IMG_OBJS_ERROR_PATH, SQBaseIcons.getImageDescriptor(DBIcon.SMALL_ERROR));

        FontPreferenceOverrides.overrideFontPrefValues(fontOverrides);
            
/*
        // Set default resource encoding to UTF-8
        String defEncoding = DBWorkbench.getPlatform().getPreferenceStore().getString(SQBasePreferences.DEFAULT_RESOURCE_ENCODING);
...
*/
    ...
    private void patchJFaceIcons() {
        final Map<String, ImageDescriptor> icons = Map.of(
            Dialog.DLG_IMG_MESSAGE_INFO, SQBaseIcons.getImageDescriptor(DBIcon.SMALL_INFO),
            Dialog.DLG_IMG_MESSAGE_WARNING, SQBaseIcons.getImageDescriptor(DBIcon.SMALL_WARNING),
            Dialog.DLG_IMG_MESSAGE_ERROR, SQBaseIcons.getImageDescriptor(DBIcon.SMALL_ERROR)
        );
    ...
    public static boolean closeOpenEditors(IWorkbenchWindow window, boolean forceRevert, boolean showConfirmation) {
        if (showConfirmation && !forceRevert &&
                !MessageDialogWithToggle.NEVER.equals(ConfirmationDialog.getSavedPreference(SQBasePreferences.CONFIRM_EXIT))
        ) {
            // Workaround of #703 bug. NEVER doesn't make sense for Exit confirmation. It is the same as ALWAYS.
            if (ConfirmationDialog.confirmAction(window.getShell(), SQBasePreferences.CONFIRM_EXIT, ConfirmationDialog.QUESTION)
    ...
    private void resetPerspectiveIfNeeded() {
        ...
        if (perspective != null && !perspective.getId().equals(SQBasePerspective.PERSPECTIVE_ID)) {
            return;
        }
        ...
        SQBaseNotifications.showNotification(
            SQBaseNotifications.NT_PERSPECTIVE_RESET,
    ...
    private static class OpenEventProcessor implements Listener {
        ...
        void catchUp() {
            ...
            IInstanceController controller = SQBaseApplication.getInstance().getInstanceServer();
            if (controller != null) {
