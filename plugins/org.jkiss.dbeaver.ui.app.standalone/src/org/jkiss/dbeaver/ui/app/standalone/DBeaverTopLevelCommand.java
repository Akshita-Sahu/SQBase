...
import org.jkiss.sqbase.model.cli.command.AbstractTopLevelCommand;
...
@CommandLine.Command(name = "sqbase", description = "SQBase commands")
public class SQBaseTopLevelCommand extends AbstractTopLevelCommand {
    private static final Log log = Log.getLog(SQBaseTopLevelCommand.class);
    ...
    @CommandLine.Option(names = {"-stop", "-quit"}, description = "Stop SQBase running instance")
    private boolean stop;

    @CommandLine.Option(names = {"-f", "-file"}, arity = "1", split = ",", description = "Open a file")
    private List<String> filesToOpen;

    // open files via double-click or "Open with SQBase"
    ...
    @CommandLine.Option(names = {"-bringToFront"}, description = "Bring SQBase window on top of other applications")
    ...
    protected SQBaseTopLevelCommand(
