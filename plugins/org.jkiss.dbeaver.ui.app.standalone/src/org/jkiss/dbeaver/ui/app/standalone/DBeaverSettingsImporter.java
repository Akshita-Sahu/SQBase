...
import org.jkiss.sqbase.model.runtime.BaseProgressMonitor;
...
class SQBaseSettingsImporter {
    ...
    private final SQBaseApplication application;
    ...
    public SQBaseSettingsImporter(SQBaseApplication application, Display display) {
    ...
    boolean migrateFromPreviousVersion(final File oldDir, final File newDir) {
        ...
        String oldVersion = workspaceProps.getProperty(SQBaseApplication.VERSION_PROP_PRODUCT_VERSION);
        if (oldVersion == null) {
            oldVersion = "3.x";
        } else {
        ...
        Image sqbaseIcon = AbstractUIPlugin.imageDescriptorFromPlugin(SQBaseApplication.APPLICATION_PLUGIN_ID, "icons/sqbase32.png").createImage();
        Image sqbaseLogo = AbstractUIPlugin.imageDescriptorFromPlugin(SQBaseApplication.APPLICATION_PLUGIN_ID, "icons/sqbase64.png").createImage();
        ...
        windowShell.setImage(sqbaseIcon);
...
//                "Make sure previous version of " + GeneralUtils.getProductName() + " isn't running"
    ...
    private void migrateWorkspace(final File oldDir, final File newDir) {
        ...
        new Thread(() -> {
            try {
                ...
                SQBaseApplication.WORKSPACE_MIGRATED = true;
    ...
    private void copyWorkspaceFiles(DBRProgressMonitor monitor, DIR_TYPE parentDirType, File fromDir, File toDir) {
        ...
        for (File file : files) {
            ...
            if (file.isDirectory()) {
                if (parentDirType == DIR_TYPE.METADATA && !(fileName.equals(".plugins") || fileName.equals("qmdb") || fileName.startsWith("sqbase"))) {
                    // Skip all dirs but plugins
                    skippedFiles += countWorkspaceFiles(file);
                    continue;
                }
                if (parentDirType == DIR_TYPE.PLUGINS) {
                    if (!fileName.contains("sqbase") && !ArrayUtils.contains(COPY_PLUGINS, fileName)) {
                        skippedFiles += countWorkspaceFiles(file);
                        continue;
                    }
                }
                ...
                if (fileName.equals("org.jkiss.sqbase.core.prefs")) {
                    // Patch configuration
                    ...
                    if (driversHomeProp != null && !driversHomeProp.isEmpty()) {
                        ...
                        if (oldDriversPath.equals(new File(oldWorkspacePath, ".metadata/.plugins/org.jkiss.sqbase.core"))) {
                    ...
                    try (Reader r = new InputStreamReader(new FileInputStream(file), GeneralUtils.UTF8_CHARSET)) {
                        ...
                        driversText = driversText.replace("${workspace}\\.metadata\\.plugins\\org.jkiss.sqbase.core", "${drivers_home}");
                        driversText = driversText.replace("${workspace}/.metadata/.plugins/org.jkiss.sqbase.core", "${drivers_home}");
                    } catch (IOException e) {
                        e.printStackTrace(System.err);
                    }
