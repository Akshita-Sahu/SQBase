...
package org.jkiss.sqbase.ui.charts;
...
import org.jkiss.sqbase.ui.charts.internal.UIChartsMessages;
...
public class BaseChartComposite extends ChartComposite {
    ...
    protected void fillContextMenu(IMenuManager manager) {
        ...
        if (zoomable != null) {
            manager.add(new Action(UIChartsMessages.base_chart_composite_action_zoom_in, SQBaseIcons.getImageDescriptor(UIIcon.ZOOM_IN)) {
                @Override
                public void runWithEvent(Event e) {
                    doZoom(zoomable, getChartCanvas().toControl(getDisplay().getCursorLocation()), getZoomInFactor());
                }
            });
            manager.add(new Action(UIChartsMessages.base_chart_composite_action_zoom_out, SQBaseIcons.getImageDescriptor(UIIcon.ZOOM_OUT)) {
                @Override
                public void runWithEvent(Event e) {
                    doZoom(zoomable, getChartCanvas().toControl(getDisplay().getCursorLocation()), getZoomOutFactor());
                }
            });
            manager.add(new Action(UIChartsMessages.base_chart_composite_action_zoom_reset, SQBaseIcons.getImageDescriptor(UIIcon.ZOOM)) {
                @Override
                public void runWithEvent(Event e) {
                    restoreAutoBounds();
                }
        ...
        if (hasConfigurationDialog()) {
            ...
            manager.add(new Action(UIChartsMessages.base_chart_composite_action_settings, SQBaseIcons.getImageDescriptor(UIIcon.CONFIGURATION)) {
                @Override
                public void run() {
        ...
        if (hasColorsConfiguration()) {
            manager.add(new Action(UIChartsMessages.base_chart_composite_action_colors, SQBaseIcons.getImageDescriptor(UIIcon.PALETTE)) {
                @Override
                public void run() {
