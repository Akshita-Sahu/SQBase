...
import org.jkiss.sqbase.model.app.DBPPlatformLanguage;
import org.jkiss.sqbase.model.app.DBPPlatformLanguageManager;
import org.jkiss.sqbase.model.preferences.DBPPreferenceStore;
import org.jkiss.sqbase.registry.SWTBrowserRegistry;
import org.jkiss.sqbase.registry.language.PlatformLanguageDescriptor;
...
import org.jkiss.sqbase.ui.editors.DatabaseEditorPreferences;
...
public class PrefPageDatabaseUserInterface extends AbstractPrefPage implements IWorkbenchPreferencePage, IWorkbenchPropertyPage {
    public static final String PAGE_ID = "org.jkiss.sqbase.preferences.main"; //$NON-NLS-1$
    ...
    private void setSettings() {
        ...
        if (isStandalone) { 
            if (!ApplicationPolicyService.getInstance().isInstallUpdateDisabled()) {
                automaticUpdateCheck.setSelection(store.getBoolean(SQBasePreferences.UI_AUTO_UPDATE_CHECK));
            }
        ...
        statusBarBreadcrumbPositionCombo.setEnabled(statusBarShowBreadcrumbsCheck.getSelection());
        statusBarShowStatusCheck.setSelection(store.getBoolean(SQBasePreferences.UI_STATUS_BAR_SHOW_STATUS_LINE));
    }
    ...
    protected void performDefaults() {
        ...
        if (isStandalone) {
            ...
            if (!ApplicationPolicyService.getInstance().isInstallUpdateDisabled()) {
                automaticUpdateCheck.setSelection(store.getDefaultBoolean(SQBasePreferences.UI_AUTO_UPDATE_CHECK));
            }
        }
        if (isWindowsDesktopClient()) {
            SWTBrowserRegistry.getActiveBrowser();
        ...
        statusBarShowStatusCheck.setSelection(store.getDefaultBoolean(SQBasePreferences.UI_STATUS_BAR_SHOW_STATUS_LINE));
    }
    ...
    public boolean performOk()
    {
        ...
        if (isStandalone) {
            ...
            if (!ApplicationPolicyService.getInstance().isInstallUpdateDisabled()) {
                store.setValue(SQBasePreferences.UI_AUTO_UPDATE_CHECK, automaticUpdateCheck.getSelection());
            } else {
                store.setValue(SQBasePreferences.UI_AUTO_UPDATE_CHECK, Boolean.FALSE);
            }
            ...
            store.setValue(SQBasePreferences.UI_STATUS_BAR_SHOW_BREADCRUMBS, breadcrumbLocation.name());
            store.setValue(SQBasePreferences.UI_STATUS_BAR_SHOW_STATUS_LINE, statusBarShowStatusCheck.getSelection());
