...
import org.jkiss.sqbase.tasks.ui.internal.TaskUIViewBundle;
        ...
        if (taskView != null) {
            ...
            switch (groupBy) {
                case category:
                    ...
                    //element.setIcon(SQBaseIcons.getImageDescriptor(DBIcon.TREE_DATABASE_CATEGORY));
                    break;
                case type:
                    element.setChecked(tasksTree.isGroupByType());
                    //element.setIcon(SQBaseIcons.getImageDescriptor(DBIcon.TREE_TASK));
                    break;
            }
            String commandName = Platform.getResourceBundle(Platform.getBundle(TaskUIViewBundle.BUDLE_ID)).getString("command.org.jkiss.sqbase.task.group." + groupBy.name() + ".name");
