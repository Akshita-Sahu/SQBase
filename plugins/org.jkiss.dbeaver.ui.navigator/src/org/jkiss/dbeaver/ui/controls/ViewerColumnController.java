...
public class ViewerColumnController<COLUMN, ELEMENT> {
    ...
    public void fillConfigMenu(IContributionManager menuManager) {
        menuManager.add(new Action(UINavigatorMessages.obj_editor_properties_control_action_configure_columns, SQBaseIcons.getImageDescriptor(UIIcon.CONFIGURATION)) {
            {
        ...
        addColumn(name, description, style, defaultVisible, required, false, null, new ColumnLabelProvider() {
            ...
            public void update(ViewerCell cell) {
                if (cell.getColumnIndex() == 0) {
                    if (defaultIcon != null) {
                        cell.setImage(SQBaseIcons.getImage(defaultIcon));
                    }
                }
