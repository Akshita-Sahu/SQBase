 ...
 */
package org.jkiss.sqbase.ui.navigator.project;
...
public class ProjectExplorerView extends DecoratedProjectView implements DBPProjectListener {
    ...
    public static final String VIEW_ID = "org.jkiss.sqbase.core.projectExplorer";
    ...
    private void createColumns(final TreeViewer viewer) {
        ...
        /*
                        ...
                        if (node instanceof DBNDatabaseNode) {
                            return SQBaseIcons.getImage(((DBNDatabaseNode) node).getDataSourceContainer().getDriver().getIcon());
                        } else if (node instanceof DBNResource) {
                            Collection<DBPDataSourceContainer> containers = ((DBNResource) node).getAssociatedDataSources();
                            if (containers != null && containers.size() == 1) {
                                return SQBaseIcons.getImage((containers.iterator().next().getDriver().getIcon()));
                            }
                        }
        */
                        return null;
