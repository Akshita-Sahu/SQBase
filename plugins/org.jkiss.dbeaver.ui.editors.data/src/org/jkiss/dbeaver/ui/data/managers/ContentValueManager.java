...
import org.jkiss.sqbase.model.data.DBDContentCached;
import org.jkiss.sqbase.model.data.DBDContentStorage;
import org.jkiss.sqbase.model.data.DBDDisplayFormat;
import org.jkiss.sqbase.model.data.storage.ExternalContentStorage;
import org.jkiss.sqbase.model.exec.DBCException;
import org.jkiss.sqbase.model.preferences.DBPPropertyManager;
...
import org.jkiss.sqbase.ui.data.IStreamValueEditorPersistent;
import org.jkiss.sqbase.ui.data.IValueController;
import org.jkiss.sqbase.ui.data.IValueEditor;
import org.jkiss.sqbase.ui.data.dialogs.TextViewDialog;
import org.jkiss.sqbase.ui.data.editors.ContentInlineEditor;
import org.jkiss.sqbase.ui.data.editors.ContentPanelEditor;
import org.jkiss.sqbase.ui.data.editors.StringInlineEditor;
import org.jkiss.sqbase.ui.dialogs.DialogUtils;
import org.jkiss.sqbase.ui.editors.content.ContentEditor;
...
public class ContentValueManager extends BaseValueManager {
    ...
    public static void contributeContentActions(
        ...
        if (controller.getValue() instanceof DBDContent content) {
            if (!content.isNull()) {
                manager.add(new Action(ResultSetMessages.model_jdbc_save_to_file_, SQBaseIcons.getImageDescriptor(UIIcon.SAVE_AS)) {
                    @Override
                    public void run() {
            ...
            manager.add(new Action("Open in external editor", SQBaseIcons.getImageDescriptor(UIIcon.FILE)) {
                @Override
                public void run() {
                    try {
            ...
            manager.add(new Action(ResultSetMessages.model_jdbc_load_from_file_, SQBaseIcons.getImageDescriptor(UIIcon.LOAD)) {
                @Override
                public void run() {
    ...
    private static void openOctetStream(byte[] data) throws IOException {
        ...
        if (data.length == 0) {
            ...
            // delete the file when the user closes the SQBase application
