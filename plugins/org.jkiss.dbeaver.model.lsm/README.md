...
`SQLStandardAnalyzer` is a common analyzer with parser and lexer by the common grammar for most SQL databases without any specific settings. It's being instantiated with the `SQLStandardAnalyzerFactory` implementation of the `LSMAnalyzerFactory` interface via extension point in `org.jkiss.sqbase.model.sql.plugin.xml`.

```xml

<extension point="org.jkiss.sqbase.lsm.dialectSyntax">
    <lsmDialect analyzerFactoryClass="org.jkiss.sqbase.model.lsm.sql.dialect.SQLStandardAnalyzerFactory">
        <appliesTo dialectClass="org.jkiss.sqbase.model.impl.sql.BasicSQLDialect"/>
    </lsmDialect>
</extension>
```
...
```xml
    <extension point="org.jkiss.sqbase.lsm.dialectSyntax">
        <lsmDialect analyzerFactoryClass="org.jkiss.sqbase.ext.sqlite.model.SQLiteSQLAnalyzer">
            <appliesTo dialectClass="org.jkiss.sqbase.ext.sqlite.model.SQLiteSQLDialect"/>
        </lsmDialect>
    </extension>
```
